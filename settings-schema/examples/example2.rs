...
pub struct VideoDesc {
    ...
    #[schema(higher_order(
        ...
        modifier(
            target = "video.render_resolution.variant",
            expr = r#""Scale""#,
        ),
        modifier(
            target = "video.render_resolution.Scale",
            expr = ""
        ),
        modifier(
            target = "video.recommended_target_resolution.variant",
            expr = r#""Scale""#,
        ),
    ))]
    //
...
pub enum SocketProtocol {
    ...
    // #[schema(advanced)]
    ThrottledUdp {
        #[schema(min = 1.0, step = 0.1, gui = "up_down")]
